...
def open_out_gen(mode, perm, path):
    assert (
        False
    ), 'The "mode" argument must be converted from a \
"type open_flag" to a "str" that is accepted by Python open'
    assert (
        False
    ), "Need to implement a way to modify the file permissions, \
        ...
        def _f2(pos):
            def _f3(length):
                if not (pos < len(buf) and (pos + length) < len(buf)):
                    raise ValueError(
                        "output : pos and length do not designate a valid substring of buf"
                    )
...
def compare(x):
    def _f(y):
        return cond(
            np.equal(x, y),
            lambda _: 0,
            cond(np.less(x, y), lambda _: -1, lambda _: 1, None),
            None,
        )
